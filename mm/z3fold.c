 ...
 * information on how th(zhdr);e allocations are organized within each z3fold page.
...
static int z3fold_reclaim_page(struct z3fold_pool *pool, unsigned int retries)
{
	...
	for (i = 0; i < retries; i++) {
		...
		if (!test_bit(PAGE_HEADLESS, &page->private)) {
			...
			/*
			 * it's safe to unlock here because we hold a reference to this
			 * page
			 */
			z3fold_page_unlock(zhdr);
		} else {
